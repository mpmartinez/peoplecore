...
public interface IAttendanceImportService
{
    ...
    /// <summary>Saves the punches of every person matched to an employee; the rest are reported, not saved.</summary>
    Task<AttendanceImportResultDto> ImportAsync(IReadOnlyList<AttendancePunchDto> punches, IReadOnlyList<string> fileErrors, CancellationToken ct = default);
