...
- **Default period:** from the day after the employee's last Paid Regular run's `PeriodEnd` (or the first of the last working day's month, if none) to the last working day. HR may set a different start, which must not be after the last working day, nor on or before the `PeriodEnd` of a Paid Regular run that included the employee: "Payroll {RunNumber} already paid up to {PeriodEnd:MMM d, yyyy}; start final pay after that."
